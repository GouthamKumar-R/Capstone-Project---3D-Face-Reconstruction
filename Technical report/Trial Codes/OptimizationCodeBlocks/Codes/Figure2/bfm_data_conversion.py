...
def transferBFM09():
	...
	idBase = idBase[:,:30] # use only first 80 basis

	exBase = expPC*np.reshape(expEV,[-1,79])
	exBase = exBase/1e5 # unify the scale to decimeter
	exBase = exBase[:,:30] # use only first 64 basis

	texBase = texPC*np.reshape(texEV,[-1,199])
	texBase = texBase[:,:30] # use only first 80 basis
	...
	idBase = np.reshape(idBase,[-1,3,30])
	idBase = idBase[index_shape,:,:]
	idBase = np.reshape(idBase,[-1,30])

	texBase = np.reshape(texBase,[-1,3,30])
	texBase = texBase[index_shape,:,:]
	texBase = np.reshape(texBase,[-1,30])

	exBase = np.reshape(exBase,[-1,3,30])
	exBase = exBase[index_exp,:,:]
	exBase = np.reshape(exBase,[-1,30])
