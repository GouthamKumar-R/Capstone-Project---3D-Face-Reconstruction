...
class ReconModel(nn.Module):
  ...
  def Split_coeff(self, coeff):
      id_coeff = coeff[:, :30]  # identity(shape) coeff of dim 80
      ex_coeff = coeff[:, 30:60]  # expression coeff of dim 64
      tex_coeff = coeff[:, 60:90]  # texture(albedo) coeff of dim 80
      angles = coeff[:, 90:93]  # ruler angles(x,y,z) for rotation of dim 3
      gamma = coeff[:, 93:120]  # lighting coeff for 3 channel SH function of dim 27
      translation = coeff[:, 120:]  # translation coeff of dim 3
...
id_tensor = torch.zeros((1, 30), dtype=torch.float32, requires_grad=True, device='cuda')
tex_tensor = torch.zeros((1, 30), dtype=torch.float32, requires_grad=True, device='cuda')
exp_tensor = torch.zeros((1, 30), dtype=torch.float32, requires_grad=True, device='cuda')
